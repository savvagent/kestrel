...
describe('Kestrel class', () => {
  ...
  it('should have certain methods and properties when instantiated', () => {
    expect(client).to.be.instanceof(Kestrel);
  ...
  it('should support instantiation with an array of interceptors', () => {
    ...
    expect(interceptors[0].id).to.be.equal('KESTREL_JSON_REQUEST');
    expect(interceptors[1].id).to.be.equal('KESTREL_JSON_RESPONSE');
  });
  ...
  it('should register a response interceptor', () => {
    ...
    const { interceptors } = client;
    expect(interceptors).to.be.an('array');
    expect(interceptors[0].id).to.be.equal('KESTREL_JSON_RESPONSE');
  });
